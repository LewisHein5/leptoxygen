...
fn read_leptonic_metadata(cargo_toml_path: &PathBuf) -> Result<Option<LeptonicMetadata>> {
    ...
    let leptonic_metadata = cargo_toml
        ...
        .and_then(|metadata| metadata.get("leptoxygen"));

    let meta = match leptonic_metadata {
        Some(metadata) => {
            // Found "leptoxygen" in either package or workspace metadata, proceed
            log(
                Level::Info,
                format!(
                    "Found 'leptoxygen' in metadata of package or workspace: {:?}",
                    metadata
                ),
            );
            metadata
